...
export default class ScheduleLoader {
  ...
  static parseStudIPEventstoTimetableEvents(data: any): ScheduleEvent[] {
    ...
    for(let weekdayIndex = 0; weekdayIndex < weekdays; weekdayIndex++){
      ...
      for (const index in dataForWeekday) {
        ...
        let event = ScheduleLoader.parseRawEventToScheduleEvent(weekdayIndex, rawEventAtWeekday);
        events.push(event);
      }
  ...
  static parseRawEventToScheduleEvent(weekdayIndex: number, rawEventAtWeekday: any): ScheduleEvent{
    let buildingAndRoom = ScheduleLoader.filterStudIPBuilding(rawEventAtWeekday.title);
    let eventJson = {
      name: ScheduleLoader.filterNameFromEventContent(rawEventAtWeekday.content),
  ...
  static filterStudIPBuilding(eventTitle: string): any{
    ...
    let regBuildingRoom =/(\w{1,2})\s?\/\s?([E,B,\d]\d{2})|(\w{1,2})\s([E,B,\d]\d{2})/g;
